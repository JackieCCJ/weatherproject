...
rl.question("Enter forecast time (YYYY-MM-DD HH:MM:SS): ", (inputTime) => {
  ...
  // Try to parse input into a Date
  let parsedInput;
  try {
    if (/^\d{4}-\d{2}-\d{2} \d{2} \d{2}$/.test(inputTime)) {
      parsedInput = new Date(inputTime + ":00");
    }
    // add ":00:00" if user only typed YYYY-MM-DD HH
    else if (/^\d{4}-\d{2}-\d{2} \d{2}$/.test(inputTime)) {
      parsedInput = new Date(inputTime + ":00:00");
    }
    // add "00:00:00" if user only typed YYYY-MM-DD
    else if (/^\d{4}-\d{2}-\d{2}$/.test(inputTime)) {
      parsedInput = new Date(inputTime + " 00:00:00");
    }
    else {
      parsedInput = new Date(inputTime);
    }
  } catch {
  ...
  const inputMs = parsedInput.getTime();
